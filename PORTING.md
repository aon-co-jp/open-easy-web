...
> aruaru-db/CLAUDE.md冒頭)。本リポジトリの管理APIをGraphQLへ移行する
> 場合も、必ずこのSET連携の価値強化になるか確認すること。
...
> セルフホスト時のアカウント設定(自分のe-mail・電話番号の登録方法)・
> 2FA(フィーチャーフォンでの確認方法含む)は [`manual-JAPAN.md`](manual-JAPAN.md)
> を参照してください。
