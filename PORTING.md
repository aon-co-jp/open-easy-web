...
`fetch`がブロックされる——この制約は`open-web-server`側にCORS対応を
追加するか、reverse proxyで同一オリジンに揃えることで解消できるが、
**このリポジトリの今回の変更ではopen-web-server側のCORS対応は行って
いない**(範囲外、次回課題)。移植先でこの機能を使う場合は、
`open-web-server`と`open-easy-web`を同一オリジン配下(reverse proxy
経由)に置くか、CORS対応を別途追加すること。
