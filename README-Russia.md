...
- **Круговые диаграммы использования памяти и диска (добавлено
  2026-07-31 / 2026-08-04)**: реальная физическая память, виртуальная
  память (swap) и фактическое использование HDD/SSD машины, на которой
  работает этот сервер, получаемые через `sysinfo` и отображаемые в виде
  круговых диаграмм и значений в GiB в интерфейсе (`/admin/system/memory`,
  `/admin/system/disk`, аутентификация `x-admin-token`).
...
> ⚠️ **Предупреждение о сборке (среды с сетевым диском)**: если этот
> репозиторий находится на смонтированном по сети диске (например, SMB),
> чтение/запись вывода `target/` от `cargo build` или ввода/вывода
> `wasm-bindgen` прямо на этом диске **может вернуть устаревшее
> содержимое сразу после записи** (несогласованность кэша чтения,
> реально произошло 2026-07-20). Если пересборка не применяется,
> направьте вывод сборки на локальный диск с помощью `cargo build
> --target-dir <локальная-временная-директория>` и запустите
> `wasm-bindgen` для этой локальной копии.
