...
**`open-runo`/`poem-cosmo-tauri`側のネイティブRust(hyperミドルウェア)
...
- **vhost生成・HTTPS自動設定**: `scripts/gen-vhost.sh` で、
  ...
  keepalive)はここには含まれない**(`open-runo`/`poem-cosmo-tauri`側の
  ネイティブRust実装が担当)。
...
- **共有バックエンドへの動的登録(「分身の術」)**: ドメインごとに
  `open-runo`/`poem-cosmo-tauri`の新規プロセスを個別インストールする
...
- **Web高速化(KUSANAGI的なgzip圧縮・静的キャッシュ・FastCGIバッファ調整・
  ...
  `open-runo`/`poem-cosmo-tauri`側のネイティブRust実装を参照)。
