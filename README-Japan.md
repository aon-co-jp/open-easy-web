...
- **システムメモリ・ディスク使用状況の円グラフ表示(2026-07-31/
  ...
  `/admin/system/disk`、`x-admin-token`認証)。
...
- **共有バックエンドへの動的登録(「分身の術」)**: ドメインごとに
  `open-runo`/RPoem(旧poem-cosmo-tauri)の新規プロセスを個別インストールする
  代わりに、既に稼働中の共有バックエンドへこのサイトのドメインを
  動的登録できる(ドメイン追加のたびにバックエンドプロセスを増やす
  必要が無い)。
- **分散同期クローンDB + ディザスタリカバリ(2026-07-25新設)**:
  「初回セットアップガイド」のStep 5で、このファイルサーバーが持つ
  サイトデータを他のVPSへ継続的に複製する「分散同期先(SFTP経由)」を
  簡単に登録・削除でき、ネット切断・非常時にはメール/Googleドライブへ
  自動フェイルオーバー(自動圧縮/自動展開込み)する退避先も設定できる。
  姉妹リポジトリ`open-raid-z`の切断耐性ジャーナル・ディザスタリカバリ・
  オフサイト退避先実装(`open_raid_z_core`)をそのまま再利用しており、
  この機能自体を再実装してはいない。**設定は任意**——スキップしても
  ファイルサーバーの通常利用は妨げられない。管理API
  (`POST`/`GET`/`DELETE /admin/dist-sync/targets`、
  `POST /admin/dist-sync/disaster-fallback`、
  `POST /admin/dist-sync/first-time-setup`)は環境変数
  `OPEN_EASYWEB_DIST_SYNC_ADMIN_TOKEN`未設定時は無効化される。
  **正直な開示**: 圧縮のGPU/NPU(DirectX)アクセラレーションは
  `open_raid_z_core::accel`と同様、2026-07時点では常にCPU実装へ安全に
  フォールバックする(未実装を実装済みと偽らない)。実VPS間・実クラウド
  アカウントを使った結合テストは行っておらず、ローカルモック
  (到達不能アドレス・実SMTP/実クラウド未接続)でのみ検証している。
  詳細は`CLAUDE.md`のHANDOFF(2026-07-25)を参照。
  **2026-07-25続き・実配線完了**: 上記の「土台」だけだったギャップを
  解消し、実際にサイトファイルをアップロードすると
  (`POST /api/sites/:name/upload`)、登録済みの分散同期先へ
  非ブロッキングで自動複製されるようになった——アップロードした
  ユーザーへの応答は複製の完了を一切待たない(遅い/到達不能なVPSが
  あってもアップロード自体は遅延しない)。同期先が1件も登録されて
  いない場合は複製処理自体が一切スケジュールされず、既存動作から
  何も変わらない。ディザスタ用退避先(Email/Googleドライブ)への
  実配線・切断耐性ジャーナル経由の書き込みとの統合は次回課題として
  残っている。詳細は`CLAUDE.md`の同日追記HANDOFFを参照。
