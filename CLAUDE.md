...
このリポジトリ、および関連プロジェクト(`open-runo`/`poem-cosmo-tauri`/
...
Nginx/Apache設定生成という形ではなく、**`open-runo`/`poem-cosmo-tauri`
...
高速化が必要な場合は`open-runo`/`poem-cosmo-tauri`のRustサーバーを
...
- 実装で迷った場合や、API仕様の詳細確認が必要な場合は、学習データからの
  推測より公式ドキュメント(上記URL)、または`open-runo`/`poem-cosmo-tauri`
...
- **このリポジトリにWeb高速化機能(gzip/静的キャッシュ/FastCGIバッファ
  ...
  `open-runo`/`poem-cosmo-tauri`側で行うこと。
