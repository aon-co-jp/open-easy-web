...
- **رسوم بيانية دائرية لاستخدام الذاكرة والقرص (أُضيف 2026-07-31 /
  2026-08-04)**: عرض الذاكرة الفعلية والذاكرة الافتراضية (swap) ومساحة
  القرص الصلب/SSD الفعلية المستخدمة على الجهاز الذي يشغّل هذا الخادم،
  عبر `sysinfo`، كرسم بياني دائري وأرقام GiB في الواجهة
  (`/admin/system/memory`، `/admin/system/disk`، مصادقة
  `x-admin-token`).
